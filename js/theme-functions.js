...
(function($) {
    $.fn.dnngomegamenu = function(m) {
        ...
        return this.each(function(index) {
            var me = $(this),
                primary = me.find(".primary-structure > li"),
                slide = ".menuslide",
                subs = ".sub-menu",
                subbox = "boxslide",
                hover = "menu_hover",
                slidedefault = "slide-menu",
                interval, interval2;
            if (rtl) {
                me.addClass("rtl")
            }
            ...
            primary.find("li").mouseenter(function() {
                ...
                if (subbox.css("display") == "none") {
                    ...
                    sub_left = sub_l + $(this).width(), winwidth = $(window).width(), sub_width = subbox.width();
                    if (rtl) {
                        if (sub_l < sub_width) {
                            subbox.css({
                                "left": "100%",
            ...
            function roller(e, defaultTop) {
                if (e.offset().top + e.height() - $(window).scrollTop() > $(window).height()) {
                    ...
                    up.insertBefore(e.children("ul,div.boxslide")).hide();
                    down.insertAfter(e.children("ul,div.boxslide"));
                    if (e.hasClass("sub-menu") || m.popUp == "level") {
                        p_height = 0
                    }
                    if (h < w_h) {
                        min_top = p_height;
            ...
            me.find(".menuslide,.sub-menu").each(function(index, element) {
                var e = $(this),
                ...
                rollsubmenu.each(function() {
                    if ($(this).height() > $(window).height() - e.height()) {
                        $(this).css({
                            ...
                            "overflow": "auto",
                            "marginRight": "-20px",
                            "width": $(this).parent(".menuslide").width() + 18
                        });
...
(function($) {
    ...
    jQuery.fn.dynamicnumbers = function(number, time, speed) {
        ...
        var dynamic = function() {
            if (i < numbers) {
                ...
                $el.data("separator") ? $el.text(number.replace(/(\d)(?=(?:\d{3})+$)/g, "$1,")) : $el.text(number);
                clearTimeout(interval)
            }
        };
    ...
    var addAnimation = function(element) {
        $(element).each(function(i, el) {
            var el = $(el);
            if (el.visible(true) && !el.hasClass("animated")) {
                ...
                if (el.hasClass("dg-typewriter")) {
                    ...
                    var printer = function() {
                        if (index < l) {
                            ...
                            index++;
                            setTimeout(printer, time)
                        } else {
                            if (loop || p_index < p_length - 1) {
                                setTimeout(delete_printer, delay)
                            } else {
                    ...
                    var delete_printer = function() {
                        if (index > 0) {
                            el.html(html.slice(0, index - 1));
                            index--;
                            setTimeout(delete_printer, 30)
                        } else {
                            p_index <= p_length - 2 ? p_index++ : p_index = 0;
                            html = array[p_index];
                            l = html.length;
                ...
                if (el.hasClass("icon-svg") && !el.attr("id")) {
                    var id = "icon-svg-" + i;
                    $(this).attr("id", id);
                    var icon = el.attr("class");
                    icon = icon.substr(icon.indexOf("fa-") + 3, 40);
                    icon = icon.substr(0, icon.indexOf(" "));
                    if (typeof Vivus == "function") {
                        new Vivus("icon-svg-" + i, {
                            duration: el.data("duration") ? el.data("duration") : 120,
                            file: "icon-svg/" + icon + ".svg",
                            type: el.data("type") ? el.data("type") : "sync"
                        })
                    }
                }
                el.removeClass("visible").addClass("animated");
                if (el.hasClass("animationhover")) {
    ...
    $(window).load(function() {
        addAnimation(".animation,.animationhover")
    });
...
(function($, window, undefined) {
    ...
    $('[data-event="hover"]').dropdownHover();
})(jQuery, this);
